...
object BlockchainAPI {
  ...
  def routes(blockchain: Blockchain[IO]): HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case req@POST -> Root / "faucet" =>
      req.as[FaucetRequest].flatMap { faucetReq =>
    ...
    case req@POST -> Root / "transfer" =>
      req.as[TransferRequest].flatMap { transferReq =>
        for {
    ...
    case POST -> Root / "mine" =>
      blockchain.updateState().flatMap {
    ...
    case GET -> Root / "transaction" / hash =>
      blockchain.currentState.flatMap { ledger =>
